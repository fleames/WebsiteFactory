...
type Mode     = 'guided' | 'freetext' | 'import';
type Lang     = 'DK' | 'DE' | 'EN';
  ...
  async function handleImport() {
    ...
    try {
      ...
      const data = await res.json() as {
        name?: string; type?: string; city?: string; phone?: string;
        services?: string[]; brief?: string; error?: string;
      };
      ...
      setGuided(g => ({
        ...
        type:     data.type     || g.type,
        city:     data.city     || g.city,
        phone:    data.phone    || g.phone,
        services: data.services?.length ? data.services : g.services,
      }));
      if (data.brief) setFreetext(data.brief);
      setImportedFrom(url);
      setMode('guided');
    } catch (e) {
    ...
    <div className="w-[340px] shrink-0 bg-slate-900 border-l border-white/[0.06] flex flex-col overflow-hidden">
      ...
      <div className="flex shrink-0 border-b border-white/[0.06]">
        ...
        <button onClick={() => setMode('import')}
          className={`flex-1 py-2.5 text-xs font-semibold transition-colors ${mode === 'import' ? 'text-emerald-400 border-b-2 border-emerald-500 -mb-px bg-emerald-500/[0.04]' : 'text-slate-500 hover:text-slate-300'}`}>
          ↗ Import URL
        </button>
      </div>
        ...
        {mode === 'import' && (
          <div className="px-4 py-4 space-y-4">
            ...
            <div className="rounded-xl bg-emerald-500/[0.06] border border-emerald-500/20 px-3.5 py-3">
              <p className="text-[11px] text-emerald-300 font-semibold mb-0.5">Importer eksisterende hjemmeside</p>
              <p className="text-[10px] text-slate-500 leading-relaxed">
                Indsæt URL'en på kundens nuværende website. Vi udtrækker automatisk firmanavn, ydelser, kontaktinfo og mere — klar til at generere en ny, forbedret side.
              </p>
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="https://kundens-side.dk"
                  disabled={importing}
            ...
            {importedFrom && !importError && (
              <div className="px-3 py-2.5 bg-emerald-900/20 border border-emerald-800/30 rounded-xl">
                <p className="text-[10px] text-emerald-400 font-semibold mb-0.5">Importeret</p>
                <p className="text-[10px] text-slate-500 truncate">{importedFrom}</p>
                <p className="text-[10px] text-slate-600 mt-1">Formularen er udfyldt — kontrollér og generer.</p>
              </div>
            )}
            ...
            >
              {importing ? (
                <span className="flex items-center justify-center gap-2">
                  ...
                  Henter og analyserer…
                </span>
              ) : '↗ Importer indhold'}
            </button>

            <div className="space-y-2 pt-1">
              <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest">Hvad hentes automatisk</p>
              {[
                ['Firmanavn & branche', '🏢'],
                ['By / lokation', '📍'],
                ['Ydelser & services', '🔧'],
